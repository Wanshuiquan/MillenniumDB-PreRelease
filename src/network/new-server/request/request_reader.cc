...
void RequestReader::set_request(const uint8_t* request_bytes_, const size_t request_size_) {
...
Protocol::RequestType RequestReader::read_request_type() {
...
std::string RequestReader::read_string() {
...
uint_fast32_t RequestReader::read_size() {
    check_remaining_bytes(4);
    uint32_t value = 0;
    for (auto i = 0; i < 4; ++i) {
        value <<= 8;
...
void RequestReader::check_datatype(Protocol::DataType expected) {
    check_remaining_bytes(1);
...
void RequestReader::check_remaining_bytes(uint_fast32_t expected) const {
