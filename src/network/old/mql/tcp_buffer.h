...
#include "network/old/mql/communication_protocol.h"
...
public:
    ...
    ~TcpBuffer() {
        if (sock.is_open()) {
            ...
            // similar to send() but without throwing
    ...
    void set_status(CommunicationProtocol::StatusCodes status) {
        buffer[0] = static_cast<unsigned char>(status);
    }
