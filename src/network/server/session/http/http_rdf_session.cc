...
HttpRdfSession::HttpRdfSession(Server<Protocol::RDF_MODEL_ID>&    server,
                               stream_type&&                      stream,
...
void HttpRdfSession::run() {
    boost::asio::dispatch(stream.get_executor(),
                          beast::bind_front_handler(&HttpRdfSession::handle_request, this->shared_from_this()));
}


void HttpRdfSession::handle_request() {
    ...
    if (request.target().rfind("/update", 0) != std::string::npos) {
        is_update = true;
    } else if (request.target().rfind("/sparql", 0) == std::string::npos) {
        response_ostream << "HTTP/1.1 404 Not Found\r\n"
                         << "\r\n";
        return;
    }

    auto&& [query, response_type] = SPARQL::RequestHandler::parse_request(request);
    ...
    if (is_update) {
        // TODO: execute_update_query(query, response_ostream);
        return;
    } else {
        execute_readonly_query(query, response_ostream, response_type);
    }
}
