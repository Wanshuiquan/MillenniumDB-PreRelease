...
void MDBServer::Server<ModelId>::browser_session(tcp::socket&& socket) {
    ...
    while (true) {
        ...
        std::string path = path_cat(MDBServer::Protocol::DEFAULT_BROWSER_PATH, req.target());
        if (req.target().back() == '/')
...
void Server<ModelId>::browser_listener(asio::io_context* browser_io_context, int port) {
    ...
    while (true) {
        ...
        std::thread new_thread(browser_session, std::move(socket));
        new_thread.detach();
    }
}
...
void Server<ModelId>::run(unsigned short       port,
    ...
    Listener<ModelId> listener(*this, io_context, tcp::endpoint(tcp::v4(), port), query_timeout);
...
template class MDBServer::Server<MDBServer::Protocol::QUAD_MODEL_ID>;
template class MDBServer::Server<MDBServer::Protocol::RDF_MODEL_ID>;
