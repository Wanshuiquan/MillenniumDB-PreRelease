...
public:
        ...
        antlr4::ANTLRErrorListener* error_listener = nullptr)
    {
        ...
        res->accept_visitor(translate_property_paths);
        logger(Category::LogicalPlan, 1) << "Translating property paths:\n" << *res;
        ...
        logger(Category::LogicalPlan, 1) << "Replacing unscoped variables:\n" << *res;

        use_all_rewrite_rules(res);
        logger(Category::LogicalPlan, 1) << "Applying all rewrite rules:\n" << *res;

        ChangeJoinToSequence join_to_sequence_visitor;
        res->accept_visitor(join_to_sequence_visitor);
        logger(Category::LogicalPlan, 1) << "Changing join to sequence:\n" << *res;


        RewriteFilterSubqueries rewrite_filter_subqueries;
        res->accept_visitor(rewrite_filter_subqueries);
        logger(Category::LogicalPlan, 1) << "Rewriting filter subqueries:\n" << *res;
