...
class OpProjectSimilarity : public Op {
public:
    ...
    OpProjectSimilarity(std::unique_ptr<Op>  op_,
                        VarId                object_var_,
    ...
    bool operator<(const OpProjectSimilarity& other) const {
        if (object_var < other.object_var) {
            ...
            return false;
        } else if (metric_type < other.metric_type) {
            return true;
        } else if (other.metric_type < metric_type) {
            return false;
        } else {
    ...
    std::unique_ptr<Op> clone() const override {
        ...
        return std::make_unique<OpProjectSimilarity>(op->clone(),
                                                     object_var,
                                                     similarity_var,
