...
void CLI::ProcessSPARQLQuery(std::ostream& os, const std::string& query) {
    try {
        ...
        get_query_ctx().thread_info.time_start = start_time;
        get_query_ctx().thread_info.timeout = start_time + timeout;
        get_query_ctx().thread_info.worker_index = 0;
        ...
        logical_plan = SPARQL::QueryParser::get_query_plan(query, &error_listener);
...
void CLI::ProcessMQLQuery(std::ostream& os, const std::string& query) {
    try {
        ...
        std::unique_ptr<Op> logical_plan;
        logical_plan = MQL::QueryParser::get_query_plan(query, &error_listener);
