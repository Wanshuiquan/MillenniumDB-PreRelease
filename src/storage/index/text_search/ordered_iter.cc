    ...
    second_file_id (buffer_manager.get_tmp_file_id())
{
    ...
    run->reset();
    std::vector<ObjectId> object_ids(1);
    ...
    while (index_iter->next()) {
        ...
        object_ids[0] = ObjectId(index_iter->get_table_pointer());
        run->add(object_ids);
    }
    run->sort();
    total_pages++;
...
bool OrderedIter::next() {
    ...
    table_pointer = run->get(page_position)[0].id;
    page_position++;
    return true;
}
...
std::unique_ptr<TupleIdCollection> OrderedIter::get_run(PPage& run_page) {
    return std::make_unique<TupleIdCollection>(
        run_page,
        saved_vars,
        order_vars,
        ascending,
