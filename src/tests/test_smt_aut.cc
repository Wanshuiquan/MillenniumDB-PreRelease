...
SMTAutomaton create_smt_dfa(
        std::vector<std::tuple<uint32_t, uint32_t, std::string, std::string>>& transitions,
        std::set<uint32_t> final_states)
{
    uint64_t start_state = 0;
    std::unordered_map<uint32_t, uint32_t> state2id { { start_state, 0 } };

    SMTAutomaton dfa;
    dfa.set_start(state2id[start_state]);
    dfa.from_to_connections.emplace_back();
    dfa.set_final(std::move(final_states));

    for (auto& transition : transitions) {
        auto [from, to, type, formula] = transition;
        ...
        dfa.add_transition(SMTTransition(from, to, inverse, type, formula));
    }
...
bool check_permutation(const SMTAutomaton& A, const SMTAutomaton& B, uint_fast32_t* mapping) {
    for (uint_fast32_t s = 0; s < A.get_total_states(); s++) {
        if (A.from_to_connections.size() != B.from_to_connections.size())
        {
            return false;
        }
...
bool isomorphic(const SMTAutomaton& A, const SMTAutomaton& B) {
    if (A.get_total_states() != B.get_total_states())
    {
        return false;
    }
    ...
    for (uint64_t s = 0; s < A.get_total_states(); s++) {
    ...
    {   // permutation enumeration based on https://www.quickperm.org/01example.php
        uint_fast32_t N = A.get_total_states();
...
bool check(const SMTAutomaton& A, const SMTAutomaton& B) {
    if (!isomorphic(A, B)) {
...
SMT::ExprAnd get_formula(){
    auto top =  std::make_unique<SMT::ExprConstant>(ObjectId(ObjectId::BOOL_TRUE));
    auto vec = std::vector<std::unique_ptr<SMT::Expr>>();
    vec.push_back(std::move(top));
    return SMT::ExprAnd(std::move(vec));
}
bool simple_kleene_plus() {
    ...
    auto transition2 = std::tuple( 1, 1, "a" , "true");
    auto transitions = std::vector<std::tuple<uint32_t, uint32_t, std::string, std::string>>();
    transitions.emplace_back(transition2);
    transitions.emplace_back(transition1);
    auto expected_dfa = create_smt_dfa(transitions, { 1 });
...
//
//    auto dfa          = tmp->get_smt_automaton(&get_unique_object_id);
//    auto expected_dfa = create_smt_dfa({ { 0, 0, "a", get_formula() } }, { 0 });
//    return check(dfa, expected_dfa);
//}
...
//    auto expected_dfa = create_smt_dfa({ { 0, 1, "a" } }, { 0, 1 });
//    return check(dfa, expected_dfa);
//}
...
//
//    auto dfa          = tmp->get_rpq_automaton(&get_unique_object_id);
//    auto expected_dfa = create_smt_dfa({ { 0, 1, "a" }, { 1, 1, "a" } }, { 1 });
//    return check(dfa, expected_dfa);
//}
...
//
//    auto dfa          = tmp->get_rpq_automaton(&get_unique_object_id);
//    auto expected_dfa = create_smt_dfa({ { 0, 0, "a" } }, { 0 });
//    return check(dfa, expected_dfa);
//}
...
//
//    auto dfa = tmp->get_rpq_automaton(&get_unique_object_id);
//    auto expected_dfa = create_smt_dfa(
//            { { 0, 0, "a" }, { 0, 0, "b" } },
//            { 0 }
//    );
//    return check(dfa, expected_dfa);
...
//
//    auto dfa          = tmp->get_rpq_automaton(&get_unique_object_id);
//    auto expected_dfa = create_smt_dfa({ { 0, 1, "a" }, { 0, 2, "b" }, { 1, 1, "a" } }, { 0, 1, 2 });
//    return check(dfa, expected_dfa);
//}
